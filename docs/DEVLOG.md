...
> **What this file is:** the complete, chronological, verbatim ship/playtest log for this
> project — every "SHIPPED" entry, every playtest finding, every bug root-caused, in full
> detail. It was moved out of `CLAUDE.md` on 2026-08-09 to keep that file lean: `CLAUDE.md` is
> read at the start of every session regardless of task size, while this file is meant to be
> read on demand (e.g. "what did we do about the fishing minigame," "why does X work this way").
> Nothing below was trimmed or summarized in the move — it's a straight cut-and-paste.
>
> - For the **always-true engine/naming/design rules**, read `CLAUDE.md`.
> - For **"where do things stand right now,"** read `HANDOFF.md` (short, current snapshot).
> - Detailed per-feature specs/plans live in `docs/superpowers/specs/` and
>   `docs/superpowers/plans/`; balance calls in `docs/superpowers/DECISIONS-LOG.md`.
