...
func _ready() -> void:
	...
	# reservation still doesn't cover. (2026-08-17 playtest fix): when several buffs/debuffs stack at
	# once (Summoner minions can stack Thorns + cleanse-eligible debuffs + regen + reel_surge +
	# Empowered), the status label now scrolls internally instead of clipping (fit_content = false +
	# scroll_active = true, see below) — the panel itself stays this original fixed size rather than
	# growing to fit; a bounded, scrollable status row was chosen over "clip or grow the whole panel."
	...
	_status_label.custom_minimum_size = Vector2(ROW_W, 60)  # bounded region; scrolls internally instead of growing/clipping
