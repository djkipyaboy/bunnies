...
static func description(id: StringName) -> String:
	match id:
		...
		&"crippling_shot": return "Piercing called shot; on a hit, WEAKENS the target (its outgoing damage ×0.75, 2 turns) and deals +50% bonus damage if it's Slowed, Rooted, or recently Stunned."
		# --- Seer ---
		&"hex": return "Mystic attack reel; on a hit, CURSES the target — stacking damage-over-time (Bleed-style tiers), 3 turns."
		&"foresight": return "Shields your lowest-HP ally for ~15% of your max Mana, 3 turns (auto-targets)."
		&"mana_surge": return "Self: your next spin only is EMPOWERED ×1.6 and adds up to 2 bonus reels — a one-turn damage spike."
		# --- Warden ---
		...
		&"bastion": return "Self: heavy GUARDED (incoming ×0.5), TAUNT, and THORNS — attackers take back 20% of the damage they deal, 4 turns."
		_: return ""
