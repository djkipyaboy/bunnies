...
## binds each. Panel height 332 + 14px gap (spec §2; grew from 238 on 2026-07-04 for the taller
## status-effects reservation, then 278→312 on 2026-07-31 to stop the status row itself clipping,
## then 312→332 on 2026-08-17 so the status label can scroll instead of clip when many buffs/debuffs
## stack at once — see CombatantPanel._ready). Used for both party columns.
...
func _spawn_enemy_mid_combat(id: StringName) -> Combatant:
	...
	var p := CombatantPanel.new()
	p.position = Vector2(1276.0, 80.0 + column_index * (332.0 + 14.0))
	add_child(p)
	_panels[c] = p
	p.bind(c)
	...
	hit.custom_minimum_size = Vector2(300, 332)   # full panel height (spec §3 targeting)
	hit.size = Vector2(300, 332)
...
func _build_ally_target_click_catchers() -> void:
	for c: Combatant in _turn_manager.combatants:
		...
		hit.position = panel.position
		hit.custom_minimum_size = Vector2(300, 332)
		hit.size = Vector2(300, 332)
