...
## MainPhasePlan.can_stage_extra_ability's special-cased "mana >= 1" gate (since the AbilityDef's
...
func _init() -> void:
	...
	c.resource_pool.mana = 4
	c.resource_pool.max_mana = 10
	var before_reels: int = c.turn_reels.size()

	var fired: bool = c.fire_double_or_nothing(c.weapon_type(), 5)
	_check(fired, "fire_double_or_nothing succeeds with mana > 0")
	_check(c.resource_pool.mana == 0, "all-in gamble spends 100% of current mana")
	...
	# Mana is now 0 — no more all-in gambles until regen.
	_check(not c.fire_double_or_nothing(c.weapon_type(), 5), "fire_double_or_nothing fails when mana == 0")
	...
	_check(not plan.can_stage_extra_ability(&"double_or_nothing"), "not stageable at 0 mana, despite cost=0")

	gate_c.resource_pool.mana = 1
	_check(plan.can_stage_extra_ability(&"double_or_nothing"), "stageable at 1 mana regardless of cost=0")
