...
## Vanguard L9 "Mountain Stance" (spec/task 16): a self-cast, NO-reel, ultimate-tier extra ability
...
func _init() -> void:
	...
	_check(not early_plan.can_stage_extra_ability(&"mountain_stance"), "not stageable below level 9")
	...
	_check(plan.can_stage_extra_ability(&"mountain_stance"), "stageable at level 9, affordable, no CD")
	...
	_check(c.is_on_cooldown(&"mountain_stance"), "commit put the L9 ability on cooldown")
