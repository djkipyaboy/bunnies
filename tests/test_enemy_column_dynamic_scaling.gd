...
func _process(_delta: float) -> bool:
	...
	if _frames == 2:
		...
		const ROW_H: float = 346.0  # 332.0 panel height (2026-08-17: grew from 312 for the status-scroll fix) + 14.0 gap, matching _relayout_enemy_column()
		...
		for m: Combatant in members:
			...
			_check(panel.position.y + 332.0 * panel.scale.y <= view.y, "%s panel's scaled bottom edge stays within the viewport (no off-screen bug)" % m.display_name)
