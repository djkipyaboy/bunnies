...
func _init() -> void:
	...
	TownDemo._add_boundary_walls(parent, Rect2(0, 0, 100, 50))
	_check(parent.get_child_count() == 4, "_add_boundary_walls adds exactly 4 wall segments")
	...
	# _add_solid_collider — a single StaticBody2D centered on the given rect.
	var solid_parent := Node2D.new()
	TownDemo._add_solid_collider(solid_parent, Rect2(10, 20, 40, 30))
	_check(solid_parent.get_child_count() == 1, "_add_solid_collider adds exactly one StaticBody2D")
	var solid_shape: RectangleShape2D = solid_parent.get_child(0).get_child(0).shape
	_check(solid_parent.get_child(0).position.is_equal_approx(Vector2(30, 35)), "_add_solid_collider centers on the given rect")
	_check(solid_shape.size.is_equal_approx(Vector2(40, 30)), "_add_solid_collider matches the given rect's size")

	# Regression: the exterior plaza and the interior shop floor must never occupy
	# overlapping world space. Door.toggle_areas() hides an area with
	# visible = false / PROCESS_MODE_DISABLED — neither disables physics collision in
	# Godot, so overlapping bounds would put invisible walls (and spawned NPCs) inside
	# the "closed" area's live geometry even while it's supposed to be inaccessible.
	var exterior_walled: Rect2 = TownDemo.EXTERIOR_BOUNDS.grow(TownDemo.WALL_THICKNESS)
	var interior_walled: Rect2 = TownDemo.INTERIOR_BOUNDS.grow(TownDemo.WALL_THICKNESS)
