...
## Warden L4 "Bastion" (spec/task 31): a self-cast, NO-reel, ultimate-tier extra ability (4-turn CD)
...
func _init() -> void:
	...
	_check(not early_plan.can_stage_extra_ability(&"bastion"), "not stageable below level 4")
	...
	_check(plan.can_stage_extra_ability(&"bastion"), "stageable at level 4, affordable, no CD")
	...
	_check(c.is_on_cooldown(&"bastion"), "commit put the L4 ability on cooldown")
