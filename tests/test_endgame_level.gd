...
## Regression check for the ENDGAME playtest toggle (Task 32): spawning a Combatant at level 10
...
func _init() -> void:
	for id in ClassLibrary.IDS:
		...
		c.level = 10
		_check(c.unlocked_extra_abilities().size() == cc.extra_abilities.size(), "%s: level 10 unlocks all extra_abilities" % id)
	quit()
