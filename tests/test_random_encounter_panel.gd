...
func _initialize() -> void:
	...
	good_option.good_amber_delta = 15
	...
	_check(combat_handoff.event_log_entries.has({"line": "Stranger On The Road: Sure thing (amber +15, HP -5)", "category": &"combat"}),
	...
	_check(combat_handoff.event_log_entries.has({"line": "Stranger On The Road: Risky thing (amber -10, HP -20)", "category": &"combat"}),
		"the BAD outcome logs the encounter/option/deltas tagged combat (got %s)" % str(combat_handoff.event_log_entries))

	# An amber-only outcome (no HP delta) proves the log line only includes deltas that actually
	# apply, not a fixed "amber X, HP Y" template.
	...
	gold_only_option.neutral_amber_delta = -5
	...
	_check(combat_handoff.event_log_entries.has({"line": "Haggling Test: Haggle (amber -5)", "category": &"combat"}),
		"a delta-only-in-gold outcome logs just the amber delta, no HP mention, tagged combat")
